                    ...
                    <div class="mb-6 flex justify-between items-center flex-wrap gap-3">
                        <div>
                            ...
                            <p class="text-gray-600">
                                {{ $child->user->name ?? 'Unknown Student' }} - {{ $academicYear->year_name }} -
                                {{ $term->name }}
                        ...
                        {{-- Download Report Card button — Now works for both 'locked' and 'finalized' --}}
                        <div class="flex items-center gap-3">
                            @if ($term->status === 'locked' || $term->status === 'finalized')
                                <a href="{{ route('parent.children.report-card.download', [$child->id, $term->id]) }}"
                            ...
                            @else
                                <span
                                    class="inline-flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-medium bg-gray-100 text-gray-400 cursor-not-allowed"
                                    title="Report card will be available once the term is finalized">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M12 10v6m0 0l-3-3m3 3l3-3M3 17V7a2 2 0 012-2h6l2 2h6a2 2 0 012 2v8a2 2 0 01-2 2H5a2 2 0 01-2-2z" />
                                    </svg>
                                        ...
                                        <div class="flex items-center justify-between">
                                            <div>
                                                ...
                                                <p class="text-3xl font-bold text-blue-600 mt-2">
                                                    {{ number_format($averages['midterm'], 2) }}</p>
                                            </div>
                                        ...
                                        <div class="flex items-center justify-between">
                                            <div>
                                                ...
                                                <p class="text-3xl font-bold text-green-600 mt-2">
                                                    {{ number_format($averages['endterm'], 2) }}</p>
                                            </div>
                                        ...
                                        <tr>
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <div class="text-sm font-medium text-gray-900">
                                                    {{ $studentSubject->subject->name ?? 'Unknown Subject' }}</div>
                                                <div class="text-sm text-gray-500">
                                                    {{ $studentSubject->subject->code ?? 'N/A' }}</div>
                                                @if ($studentSubject->is_elective)
                                                    <span
                                                        class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800 mt-1">
                                                        Elective
                                                    </span>
                                                @endif
                                            </td>
