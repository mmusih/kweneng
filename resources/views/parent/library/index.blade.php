<x-app-layout>
    <x-slot name="header">
        <div
            class="mt-16 p-6 bg-gradient-to-r from-purple-500 to-indigo-600 rounded-lg shadow-lg flex items-center justify-between gap-4">
            <h2 class="font-semibold text-2xl text-white leading-tight">
                Children Library Records
            </h2>

            <a href="{{ route('parent.dashboard') }}" class="text-white hover:text-indigo-100 text-sm font-medium">
                Back to Dashboard
            </a>
        </div>
    </x-slot>
                            ...
                            <tbody class="divide-y divide-gray-200 bg-white">
                                @forelse($borrowings as $borrowing)
                                    ...
                                    <tr class="{{ $isOverdue ? 'bg-red-50' : ($loop->even ? 'bg-gray-50/40' : '') }}">
                                        <td class="px-4 py-3">
                                            {{ $borrowing->student?->user?->name ?? 'N/A' }}
                                        </td>
                                        <td class="px-4 py-3">
