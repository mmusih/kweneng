<x-app-layout>
    ...
    <div class="py-8">
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8 space-y-5">
                        ...
                        @if ($currentTerm && $currentAcademicYear)
                            ...
                            @php
                                $daysLeft = (int) now()
                                    ->copy()
                                    ->startOfDay()
                                    ->diffInDays($currentTerm->end_date->copy()->startOfDay(), false);
                            @endphp

                            @if ($daysLeft > 0)
                                &middot; <span style="color:rgba(255,255,255,0.85);">{{ $daysLeft }}
                                    day{{ $daysLeft === 1 ? '' : 's' }} remaining</span>
                            @elseif($daysLeft === 0) &middot; <span
                                    style="color:rgba(255,255,255,0.85);">Last day of term</span>
                            @endif
                            @else
                                No active term
                            @endif
            ...
            @if (isset($importantAnnouncements) && $importantAnnouncements->count() > 0)
                <div id="important-announcements-section"
                    class="rounded-xl border p-4 bg-gradient-to-r from-red-50 to-orange-50 border-red-200 space-y-3">
                        ...
                        <div id="announcement-{{ $announcement->id }}"
                            ...
                            <div class="flex justify-between items-start">
                                ...
                                <div class="flex items-center gap-2 ml-2 flex-shrink-0">
                                    ...
                                    <button type="button"
                                        onclick="dismissAnnouncement({{ $announcement->id }}, 'announcement-{{ $announcement->id }}', 'important-announcements-section')"
                                        class="text-gray-400 hover:text-gray-600 transition" title="Dismiss">
            ...
            @if (($incompleteProfileChildren ?? collect())->count() > 0)
                <div class="rounded-xl border border-amber-200 bg-amber-50 p-4">
                    <div class="flex flex-col md:flex-row md:items-start md:justify-between gap-3">
                        <div>
                            <p class="text-sm font-bold text-amber-900">Student information needs updating</p>
                            <p class="text-sm text-amber-800 mt-1">
                                Please complete nationality, ID/passport/birth certificate and emergency contact details.
                            </p>
                            <div class="mt-3 flex flex-wrap gap-2">
                                @foreach (($incompleteProfileChildren ?? collect()) as $profileChild)
                                    <a href="{{ route('parent.children.profile.edit', $profileChild) }}"
                                        class="inline-flex items-center px-3 py-1.5 rounded-full bg-white text-amber-800 border border-amber-200 text-xs font-semibold hover:bg-amber-100">
                                        {{ $profileChild->user->name ?? 'Student' }}
                                    </a>
                                @endforeach
                            </div>
                        </div>
            ...
            @if (isset($announcements) && $announcements->count() > 0)
                <div id="regular-announcements-section"
                    class="rounded-xl border p-4 bg-white border-gray-200 space-y-3">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-2">
                    ...
                    <div class="space-y-2">
                        @foreach ($announcements->take(3) as $announcement)
                            <div id="announcement-{{ $announcement->id }}"
                                ...
                                <button type="button"
                                    onclick="dismissAnnouncement({{ $announcement->id }}, 'announcement-{{ $announcement->id }}', 'regular-announcements-section')"
                                    class="flex-shrink-0 text-gray-300 hover:text-gray-500 transition mt-0.5"
                                    title="Dismiss">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M6 18L18 6M6 6l12 12" />
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    @if ($event->description)
                                        <p class="text-xs text-gray-400 mt-0.5 truncate">
                                            {{ Str::limit($event->description, 60) }}</p>
                                    @endif
                                </div>
                                @php
                                    $daysUntil = (int) now()
                                        ->copy()
                                        ->startOfDay()
                                        ->diffInDays($event->start_datetime->copy()->startOfDay(), false);
                                @endphp
                                ...
                                <div class="flex items-center justify-between mb-3">
                                    <div>
                                        ...
                                        <p class="text-xs text-gray-400 mt-0.5">
                                            {{ $child->currentClass->name ?? 'No class' }} &middot;
                                            {{ $child->admission_no ?? 'N/A' }}</p>
                                    </div>
                                    @if ($isBlocked)
                                        <span
                                            class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-700">Blocked</span>
                                    @else
                                        <span
                                            class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-700">Active</span>
                                    @endif
                                </div>

                                @if (!$isBlocked && $overview)
                                    <div class="space-y-1.5 mb-3">
                                        <div class="flex justify-between text-sm">
                                            <span class="text-gray-400">Midterm avg</span>
                                            <span class="font-medium text-gray-800">
                                                {{ $overview['midterm_average'] !== null ? number_format($overview['midterm_average'], 1) : '—' }}
                                            </span>
                                        </div>
                                        <div class="flex justify-between text-sm">
                                            <span class="text-gray-400">Endterm avg</span>
                                            <span class="font-medium text-gray-800">
                                                {{ $overview['endterm_average'] !== null ? number_format($overview['endterm_average'], 1) : '—' }}
                                            </span>
                                        </div>
                                        <div class="flex justify-between text-sm">
                                            <span class="text-gray-400">Performance</span>
                                            <span class="font-medium"
                                                style="color:#3B82C4;">{{ $overview['performance_label'] }}</span>
                                        </div>
                                        <div class="flex justify-between text-sm">
            ...
            @if (($recentHomeworkRecords ?? collect())->count() > 0)
                <div class="rounded-xl border p-4 bg-white border-gray-200">
                    <div class="flex items-center justify-between mb-3">
                        <div class="flex items-center gap-2">
                            ...
                            <p class="text-sm font-semibold text-gray-800">Recent Homework</p>
                        </div>
                        <a href="{{ route('parent.homework.index') }}" class="text-xs text-cyan-600 hover:text-cyan-800 font-medium">Open homework →</a>
                    </div>
                    <div class="space-y-2">
                        @foreach (($recentHomeworkRecords ?? collect()) as $homeworkRecord)
                            <div class="flex items-start justify-between gap-3 p-3 rounded-lg hover:bg-gray-50 transition">
                                <div class="min-w-0">
                                    <p class="text-sm font-medium text-gray-900 truncate">{{ $homeworkRecord->homework->title ?? 'Homework' }}</p>
                                    <p class="text-xs text-gray-500 mt-1">
                                        {{ $homeworkRecord->student->user->name ?? 'Student' }} ·
                                        {{ $homeworkRecord->homework->subject->name ?? 'Subject' }} ·
                                        Given {{ $homeworkRecord->homework->assigned_date?->format('d M') ?? 'N/A' }}
                                    </p>
                                </div>
                                <span class="shrink-0 inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold bg-gray-100 text-gray-700">
                                    {{ \App\Models\HomeworkMark::statusLabel($homeworkRecord->submission_status) }}
                                </span>
                            </div>
                        @endforeach
                    </div>
            ...
            <div>
                ...
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    ...
                    <a href="{{ route('parent.timetable') }}"
                        class="bg-white rounded-xl border border-gray-100 p-4 hover:border-sky-300 transition">
                        <div class="h-9 w-9 rounded-lg flex items-center justify-center mb-2 bg-sky-50">
                            <svg class="h-5 w-5 text-sky-700" fill="none" viewBox="0 0 24 24"
                                stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <p class="text-sm font-medium text-gray-800">Timetable</p>
                        <p class="text-xs text-gray-400 mt-0.5">Daily lessons</p>
                    </a>
                        ...
                        <a href="{{ route('parent.children.marks.index') }}"
                            ...
                            <div class="h-9 w-9 rounded-lg flex items-center justify-center mb-2"
                                ...
                                <svg class="h-5 w-5" style="color:#2C3E6B;" fill="none" viewBox="0 0 24 24"
                                    stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V9a2 2 0 012-2h2a2 2 0 012 2v10" />
                                </svg>
                            </div>
                    ...
                    {{-- Homework quick-access --}}
                    <a href="{{ route('parent.homework.index') }}"
                        class="bg-white rounded-xl border border-gray-100 p-4 hover:border-cyan-300 transition">
                        <div class="flex items-start justify-between mb-2">
                            <div class="h-9 w-9 rounded-lg flex items-center justify-center"
                                style="background:#ECFEFF;">
                                <svg class="h-5 w-5" style="color:#0891B2;" fill="none" viewBox="0 0 24 24"
                                    stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.746 0 3.332.477 4.5 1.253v13C19.832 18.477 18.246 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" />
                                </svg>
                            </div>
                            @if (($unreadHomeworkCount ?? 0) > 0)
                                <span class="text-xs font-semibold px-1.5 py-0.5 rounded-full bg-cyan-100 text-cyan-700">
                                    {{ $unreadHomeworkCount }}
                                </span>
                            @endif
                        </div>
                        <p class="text-sm font-medium text-gray-800">Homework</p>
                        <p class="text-xs text-gray-400 mt-0.5">
                            @if (($unreadHomeworkCount ?? 0) > 0)
                                {{ $unreadHomeworkCount }} new item{{ $unreadHomeworkCount === 1 ? '' : 's' }}
                            @else
                                Subject threads
                            @endif
                        </p>
                    ...
                    <a href="{{ route('parent.messages.index') }}"
                        class="bg-white rounded-xl border border-gray-100 p-4 hover:border-indigo-300 transition">
                        <div class="flex items-start justify-between mb-2">
                            <div class="h-9 w-9 rounded-lg flex items-center justify-center"
                                style="background:#EEF2FF;">
                                <svg class="h-5 w-5" style="color:#4F46E5;" fill="none" viewBox="0 0 24 24"
                                    stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            @if (($unreadMessageCount ?? 0) > 0)
                                <span
                                    class="text-xs font-semibold px-1.5 py-0.5 rounded-full bg-indigo-100 text-indigo-700">
                                    {{ $unreadMessageCount }}
                                </span>
                            @endif
                        </div>
                        ...
                        <p class="text-xs text-gray-400 mt-0.5">
                            @if (($unreadMessageCount ?? 0) > 0)
                                {{ $unreadMessageCount }} new repl{{ $unreadMessageCount === 1 ? 'y' : 'ies' }}
                            @else
                                Contact school
                            @endif
                        </p>
                    ...
                    {{-- School Documents quick-access --}}
                    <a href="{{ route('parent.documents.index') }}"
                        class="bg-white rounded-xl border border-gray-100 p-4 hover:border-green-300 transition">
                        <div class="h-9 w-9 rounded-lg flex items-center justify-center mb-2"
                            style="background:#F0FDF4;">
                            <svg class="h-5 w-5" style="color:#16A34A;" fill="none" viewBox="0 0 24 24"
                                stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <p class="text-xs text-gray-400 mt-0.5">Timetable, booklist &amp; more</p>
                    </a>
    ...
    {{-- Announcement dismiss script --}}
    <script>
        function dismissAnnouncement(announcementId, cardId, sectionId) {
            fetch(`/parent/announcements/${announcementId}/dismiss`, {
                    method: 'POST',
                    headers: {
                ...
                .then(() => {
                    ...
                    if (card) {
                        ...
                        setTimeout(() => {
                            ...
                            if (section) {
                                const remaining = section.querySelectorAll('[id^="announcement-"]');
