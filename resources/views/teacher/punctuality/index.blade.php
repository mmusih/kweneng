<x-app-layout>
    <x-slot name="header">
        <div
            class="mt-16 p-6 bg-gradient-to-r from-amber-500 to-orange-600 rounded-lg shadow-lg flex items-center justify-between">
            <div>
                <h2 class="font-semibold text-2xl text-white leading-tight">
                    Punctuality Entry
                </h2>
                <p class="text-amber-100 text-sm mt-1">
                    Record punctuality for your class
                </p>
            </div>
            <a href="{{ route('teacher.dashboard') }}" class="text-white hover:text-amber-100 text-sm font-medium">
                Back to Dashboard
            </a>
        </div>
    </x-slot>
    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            @elseif(!$activeTerm)
                <div class="bg-yellow-50 border border-yellow-200 text-yellow-800 p-4 rounded-lg">
                    No active term found. Punctuality cannot be recorded until a term is active.
                </div>
            @else
                <div class="bg-white shadow-sm rounded-lg p-6">
                    <form method="GET" action="{{ route('teacher.punctuality.index') }}">
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            ...
                            <div>
                                ...
                                <input type="date" id="record_date" name="record_date" value="{{ $selectedDate }}"
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-amber-500 focus:ring-amber-500"
                                    required>
                            </div>
                            ...
                            <div class="flex items-end">
                                <button type="submit"
                                    class="w-full bg-amber-600 hover:bg-amber-700 text-white font-semibold py-2 px-4 rounded-md">
                                    Load Students
                                </button>
                            </div>
                        </div>
                ...
                @if ($selectedClassId)
                    <div class="bg-white shadow-sm rounded-lg p-6">
                        <div class="flex items-center justify-between mb-6">
                            <div>
                                <h3 class="text-lg font-semibold text-gray-900">Punctuality Register</h3>
                        ...
                        @if ($students->count() > 0)
                            <form method="POST" action="{{ route('teacher.punctuality.store') }}">
                                @csrf
                                                    ...
                                                    <td class="px-4 py-3 text-sm">
                                                        ...
                                                        <select name="students[{{ $index }}][status]"
                                                            ...
                                                            <option value="on_time"
                                                                {{ old("students.$index.status", $student->existing_status) === 'on_time' ? 'selected' : '' }}>
                                                                On Time</option>
                                                            <option value="late"
                                                                {{ old("students.$index.status", $student->existing_status) === 'late' ? 'selected' : '' }}>
                                                                Late</option>
                                                            <option value="very_late"
                                                                {{ old("students.$index.status", $student->existing_status) === 'very_late' ? 'selected' : '' }}>
                                                                Very Late</option>
                                                            <option value="absent"
                                ...
                                <div class="mt-6 flex justify-end">
                                    <button type="submit"
                                        class="bg-amber-600 hover:bg-amber-700 text-white font-semibold py-2 px-6 rounded-md">
                                        Save Punctuality
                                    </button>
                                </div>
                            </form>
