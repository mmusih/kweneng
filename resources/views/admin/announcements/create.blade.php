        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white kw-panel overflow-hidden">
                <div class="p-6 bg-white border-b border-gray-200">
                    <form method="POST" action="{{ route($announcementRoutePrefix . '.announcements.store') }}">
                        @csrf
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <select id="audience" name="audience"
                                        ...
                                        <option value="all_parents" {{ old('audience') == 'all_parents' ? 'selected' : '' }}>All Parents</option>
                                        <option value="form_level" {{ old('audience') == 'form_level' ? 'selected' : '' }}>Form Level</option>
                                        <option value="class_group" {{ old('audience') == 'class_group' ? 'selected' : '' }}>Class Group</option>
                                        <option value="specific_parent" {{ old('audience') == 'specific_parent' ? 'selected' : '' }}>Specific Parent</option>
                                    </select>
                            ...
                            <div id="form-level-targets" class="target-field hidden">
                                ...
                                <select id="target_form_levels" name="target_form_levels[]" multiple
                                    ...
                                    @foreach ($formLevels as $level)
                                        <option value="{{ $level }}"
                                            {{ in_array($level, old('target_form_levels', [])) ? 'selected' : '' }}>
                                            {{ $level }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                <select id="target_class_ids" name="target_class_ids[]" multiple
                                    ...
                                    @foreach ($classes as $class)
                                        <option value="{{ $class->id }}"
                                            {{ in_array($class->id, old('target_class_ids', [])) ? 'selected' : '' }}>
                                            {{ $class->name }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                <select id="target_parent_ids" name="target_parent_ids[]" multiple
                                    ...
                                    @foreach ($parents as $parent)
                                        <option value="{{ $parent->id }}"
                                            {{ in_array($parent->id, old('target_parent_ids', [])) ? 'selected' : '' }}>
