        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white kw-panel overflow-hidden">
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <form method="POST" action="{{ route($eventRoutePrefix . '.events.update', $event) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="space-y-6">
                            ...
                            <div>
                                ...
                                <textarea id="description" name="description" rows="3" class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('description', $event->description) }}</textarea>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <x-text-input id="start_datetime" class="block mt-1 w-full" type="datetime-local" name="start_datetime" :value="old('start_datetime', $event->start_datetime?->format('Y-m-d\\TH:i'))" required />
                                ...
                                <div>
                                    ...
                                    <x-text-input id="end_datetime" class="block mt-1 w-full" type="datetime-local" name="end_datetime" :value="old('end_datetime', $event->end_datetime?->format('Y-m-d\\TH:i'))" />
                                ...
                                <div>
                                    ...
                                    <select id="visibility" name="visibility" class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                                        ...
                                        @foreach (['all' => 'All Users / Whole School', 'parents' => 'Parents Only', 'teachers' => 'Teachers Only', 'students' => 'Students Only', 'specific_class' => 'Specific Class'] as $value => $label)
                                            <option value="{{ $value }}" {{ old('visibility', $event->visibility) === $value ? 'selected' : '' }}>{{ $label }}</option>
                                        @endforeach
                                    </select>
                            ...
                            <div>
                                ...
                                <select id="academic_year_id" name="academic_year_id" class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                    ...
                                    @foreach ($academicYears as $year)
                                        <option value="{{ $year->id }}" {{ old('academic_year_id', $event->academic_year_id) == $year->id ? 'selected' : '' }}>{{ $year->year_name }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div id="class-field" class="hidden">
                                ...
                                <select id="class_id" name="class_id" class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                    ...
                                    @foreach ($classes as $class)
                                        <option value="{{ $class->id }}" {{ old('class_id', $event->class_id) == $class->id ? 'selected' : '' }}>{{ $class->name }} {{ $class->academicYear?->year_name ? ' - ' . $class->academicYear->year_name : '' }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <label class="flex items-center">
                                    <input id="is_all_day" type="checkbox" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500" name="is_all_day" value="1" {{ old('is_all_day', $event->is_all_day) ? 'checked' : '' }}>
                                    <span class="ml-2 block text-sm text-gray-900">All Day Event</span>
                                </label>
                                <label class="flex items-center">
                                    <input id="is_recurring" type="checkbox" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500" name="is_recurring" value="1" {{ old('is_recurring', $event->is_recurring) ? 'checked' : '' }}>
                            ...
                            <div id="recurrence-field" class="hidden">
                                ...
                                <select id="recurrence_pattern" name="recurrence_pattern" class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                    ...
                                    @foreach (['daily' => 'Daily', 'weekly' => 'Weekly', 'monthly' => 'Monthly'] as $value => $label)
                                        <option value="{{ $value }}" {{ old('recurrence_pattern', $event->recurrence_pattern) === $value ? 'selected' : '' }}>{{ $label }}</option>
                                    @endforeach
                                </select>
